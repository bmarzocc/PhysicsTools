...
printJetFlavour::printJetFlavour(const edm::ParameterSet& iConfig)
{
  ...
  jettxt.open ("jettxt.txt", std::fstream::in | std::fstream::out | std::fstream::app);
  njets = 0;
  njets_f = 0;
...
void printJetFlavour::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for ( JetFlavourMatchingCollection::const_iterator j  = theTagByValue->begin();
    ...
    if(aFlav.getFlavour() != 0) njets_f++;
    njets++;
    jettxt << iEvent.luminosityBlock() << " " << iEvent.id().event() << " "  << aJet.get()->pt() << " " << aJet.get()->eta() << " " << aJet.get()->phi() << " " << aJet.get()->energy() << " " << aFlav.getFlavour() << " " << njets << " " << njets_f << std::endl;
